...
import { useRatingByCountStore } from "@/stores/ratingStoreByCount.js";
...
const rateCountStore = useRatingByCountStore();
...
const headers = [
  { title: "Date", align: "left", key: "food_sched.date", width: "100" },
...
const fetch = async () => {
  ...
  if (searchClicked.value) {
    rateCountStore.set_report(data.data);
    rateCountStore.set_reportDate(date);
...
<template>
  ...
  <v-row>
    <v-col cols="12" md="10">
      <v-btn class="no-print" @click="openNewTab('/ratingcountprintreport')"
        >print</v-btn
      >
    </v-col>
  </v-row>
