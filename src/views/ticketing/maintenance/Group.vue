...
const apiticketing = new Resource("ticketing/maintenance/group");
...
<template>
  ...
  <v-row justify="start">
    <v-col cols="12" md="9">
      <v-btn
        @click="dialog = !dialog"
        :disabled="!authStore.check('view-ticket-maintenance-group-can-add')"
        >New Group</v-btn
      >
    </v-col>
    <v-col cols="12" md="3">
    ...
    <v-dialog v-model="dialog" width="500">
      <v-card>
        <v-form validate-on="submit lazy" @submit.prevent="save">
          <v-card-title>
            <span class="text-h5 pa-2">Group</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              variant="text"
              :disabled="
                !authStore.check('view-ticket-maintenance-group-can-add') &&
                !authStore.check('view-ticket-maintenance-group-can-edit')
              "
              type="submit"
            >
              Save
  ...
  >
    <template v-slot:item.actions="{ item }">
      <v-icon
        size="small"
        class="me-2"
        :disabled="!authStore.check('view-ticket-maintenance-group-can-edit')"
        @click="editItem(item)"
      >
        mdi-pencil
      </v-icon>
