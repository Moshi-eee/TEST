...
<template>
  <h2 class="mb-3">Business Units</h2>
  <v-row justify="start">
    <v-col cols="4">
      <v-btn
        @click="dialog = !dialog"
        :disabled="
          !authStore.check('view-employee-maintenance-business-units-can-add')
        "
        >Add New</v-btn
      >
    </v-col>
  ...
  <v-row>
    <v-dialog v-model="dialog" persistent width="500">
      <v-card>
        <v-card-title>
          <span class="text-h5 pa-2">Business Units</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            :disabled="
              !authStore.check(
                'view-employee-maintenance-business-units-can-add'
              )
            "
            @click="save"
          >
          ...
          <v-btn
            ...
            :disabled="
              !authStore.check(
                'view-employee-maintenance-business-units-can-edit'
              )
            "
            @click="update"
          >
  ...
  >
    <template v-slot:item.actions="{ item }">
      <v-icon
        ...
        class="me-2"
        :disabled="
          !authStore.check('view-employee-maintenance-business-units-can-edit')
        "
        @click="editItem(item)"
      >
        mdi-pencil
