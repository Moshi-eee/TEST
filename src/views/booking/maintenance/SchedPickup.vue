...
const apibusinessunit = new Resource("booking/maintenance/sched_pickup");
const apiLocation = new Resource("booking/maintenance/pick-drop-location");
...
const fetchLocation = async () => {
  const { data } = await apiLocation.list({
    limit: 1000,
  });
...
const editItem = async (e) => {
  ...
  itemid.value = e.id;
  form.value = {
    sched_location_id: e.sched_location_id,
    code: e.code,
    name: e.name,
    status: e.status,
  };
...
<template>
  <h2 class="mb-3">Drop & Pickup Location</h2>
  <v-row justify="start">
    <v-col cols="4">
  ...
  <v-row>
    <v-dialog v-model="dialog" persistent width="500">
      <v-card>
        <v-card-title>
          <span class="text-h5 pa-2">Drop & Pickup Location</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-select
                  ...
                  variant="underlined"
                  :items="location"
                  v-model="form.sched_location_id"
                ></v-select>
              </v-col>
              <v-col cols="12">
                <v-text-field
