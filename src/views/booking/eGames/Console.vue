...
const bookconsole = async () => {
  try {
    const { data } = await sportApi.store({
      sport_id: "console",
      booked_datetime: form.value.booked_datetime,
      duration:
        form.value.duration == "1 Hour"
          ? 60
          : form.value.duration == "2 Hours"
          ? 120
          : 0,
...
<template>
  <v-card class="mb-4" color="#2196f3"
    ><v-card-title>
      <h2 class="text-center">GAME CONSOLE</h2>
    </v-card-title>
  </v-card>
  ...
  <v-row>
    <v-col cols="12" md="4">
      <v-card class="mx-auto my-" max-width="600">
        ...
        <v-col cols="12" sm="6" md="12" class="text-right">
          <v-btn density="comfortable" class="save" @click="bookconsole()">
            Save
          </v-btn>
        </v-col>
      </v-card>
    ...
    <v-col cols="12" md="8"
      ><v-card>
        <h5 class="text-center">Game Console Schedule</h5>
