...
const headers = [
  ...
  { title: "Category Code", align: "start", key: "code", sortable: false },
...
const fetch_category = async () => {
  loading.value = true;
  const { data } = await asset_category_api.list({
    search: search.value,
...
const save = async () => {
  if (form.value.id == null) {
    const res = await asset_category_api.store(form.value);
  } else {
    const res = await asset_category_api.update(form.value.id, form.value);
  }
  Close();
};
watch(search, () => {
    ...
    <v-dialog v-model="dialog" width="600px">
      <v-card>
        <v-form validate-on="submit lazy" @submit.prevent="save">
          <v-card-title>
            <span class="text-h5 pa-2">New Category</span>
          </v-card-title>
          <br />
